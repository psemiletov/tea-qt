...
{
  zip = new QuaZip (zipName, QTextCodec::codecForLocale()->name());
  Q_CHECK_PTR (zip);
}
                       ...
                       int windowBits, int memLevel, int strategy)
{
  ...
  if ((mode & WriteOnly) && ! (mode & ReadOnly))
    ...
    setZipError (zipOpenNewFileInZip3 (zip->getZipFile(),
                 zip->getFileNameCodec()->fromUnicode (info.name).constData(),
                 &info_z,
...
qint64 QuaZipFile::csize() const
{
  ...
  if (! zip || zip->getMode() != QuaZip::mdUnzip)
      return -1;
...
void QuaZipFile::close()
{
  ...
  if (openMode() & ReadOnly)
      ...
      if (openMode() & WriteOnly)
         if (isRaw())
             setZipError (zipCloseFileInZipRaw (zip->getZipFile(), uncompressedSize, crc));
         else
             setZipError (zipCloseFileInZip (zip->getZipFile()));
      else
          {
