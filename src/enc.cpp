...
char* CTextConverter::ConvertFromUTF16ToKOI8R(const UTF16TEXT* utf16Text)
{
  ...
  //char* KOIText = new char[len + 1]; // Учитываем нулевой символ
...
/*
char* CTextConverter::CTextConverter::ConvertFromUTF16ToKOI8R(const UTF16TEXT * utf16Text)
{
  std::string koi8rText;

  for (int i = 0; utf16Text[i] != 0; ++i)
     {
      auto it = unicode_to_koi8r.find(utf16Text[i]);

      if (it != unicode_to_koi8r.end())
          {
            koi8rText.push_back(it->second);
          } else {
            // Обработка случая, когда символ Unicode не найден в таблице
            koi8rText.push_back('?');
        }
    }

    char *result = new char[koi8rText.length() + 1];

  std::strncpy (result, koi8rText.c_str(), koi8rText.size());
